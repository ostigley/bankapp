...
module Transactions
  class DateReduce
    ...
    def self.by_month(date_sorted_transactions)
      ...
      transaction_hash = date_sorted_transactions.each_with_object(months_object) do |tx, obj|
        month_year = tx.transaction_date.strftime('%b, %Y')
        obj[month_year] += tx.amount.round
      end

      transaction_hash.each {|k,v| transaction_hash[k] = v.abs}
    end

    def self.by_day(date_sorted_transactions)
      start_date = date_sorted_transactions.first.transaction_date
      end_date = date_sorted_transactions.last.transaction_date
      days_object = DateHash.days(start_date, {}, end_date)
